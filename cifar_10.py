...
def horizontal_flip(img, axis):
    '''
    image flip horizontally by 50% probability
    img: 3D image array
...
def whiten_img(img_batch):
    ...
    for i in range(len(img_batch)):
        ...
        #  in case std = 0, we should adjust the std value, so give minimum value for it
        std = np.max([np.std(img_batch[i, :]), 1.0/IMG_HEIGHT*IMG_WIDTH*IMG_CHANNEL])
...
def random_crop_flip(batch_data, padding_size):
    ...
    for i in range(len(batch_data)):
        ...
        # flip by possibility
        crop_batch[i, :] = horizontal_flip(batch_data[i, :], axis = 1)
        
    return crop_batch

def cifar(data_path, total_sample, batch_size = 10000):
    '''
    # create cifar dataset from data path
    '''
    total_batches = total_sample/batch_size
...
def train(batch_size = 10000, pad_size): 
    '''
    train data, get from 50000 training set
    '''
    total_sample = 50000
    ...
    train_data, train_labels = cifar(data_path, total_sample, batch_size)
    padding = ((0, 0), (pad_size, pad_size), (pad_size, pad_size), (0, 0))  # do padding
    train_data = np.pad(train_data, pad_width = padding, mode = 'constant', constant_values = 0)
    train_data, train_labels = get_batch(train_data, train_labels, batch_size)
    
    return train_data, train_labels
    
    
    
def test(batch_size = 10000): 
    '''
    test data, get from 10000 testing set
    '''
    total_sample = 10000
    file_path = os.listdir(data_dir)
